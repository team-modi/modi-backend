...
class RemindV1ControllerTest {
	...
	void setUp() {
		...
		Long placeId = exhibitionPlaceRepository.save(modi.backend.domain.exhibition.ExhibitionPlace.createFromList(
				"동작아트갤러리", null, null, null, null)).getId();
		...
		// 작가는 조인(N:M)에서 조립되므로 명시적으로 연결한다(리마인드 응답 artist 조립 경로 검증).
		modi.backend.domain.exhibition.Artist artist = artistRepository.save(
				modi.backend.domain.exhibition.Artist.create("김미경 외 10인"));
