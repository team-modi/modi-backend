 ...
 * {@code culture_detail_response}에 쓰이기만 하고 아무도 읽지 않았다. 이제 선별을 전시 아웃박스
 * ({@link OutboxMessageType#FETCH_DETAIL})<b>읽기</b>로 배선한다: 도래한 작업만 집어 상세를 다시 조회한다.
 *
 ...
 * 최대 시도 초과는 RETRYABLE도 PERMANENT로 승격한다({@link OutboxFailures}).
 */
@Component
...
public class DetailEnricher {
	...
	/**
	 * 도래한 FETCH_DETAIL 작업을 배치로 처리한다. 스테디 상태(재시도 없음)에선 도래 작업이 없어 외부 호출 없이 끝난다.
	 *
	 * @return 이번 실행에서 상태를 전이시킨(성공·실패) 작업 수(낙관락 skip 제외)
	 */
	public int enrichDetails() {
		LocalDateTime now = LocalDateTime.now();
		List<OutboxMessage> jobs = exhibitionOutboxFacade.findDue(OutboxMessageType.FETCH_DETAIL, properties.batchSize(), now);
		int processed = 0;
	...
	private boolean processOne(OutboxMessage job, LocalDateTime now) {
		...
		if (state == DetailTargetState.MISSING) {
			...
			return OutboxProcessing.fail(exhibitionOutboxFacade, job, OutboxFailureType.RETRYABLE,
